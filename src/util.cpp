...
const char * const BITCOIN_CONF_FILENAME = "salen.conf";
const char * const BITCOIN_PID_FILENAME = "salend.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "salen" is a composite category enabling all Salen-related debug output
            if(ptrCategory->count(string("salen"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SalenCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SalenCore
    // Mac: ~/Library/Application Support/SalenCore
    // Unix: ~/.salencore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SalenCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SalenCore";
#else
    // Unix
    return pathRet / ".salencore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty salen.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override salen.conf
